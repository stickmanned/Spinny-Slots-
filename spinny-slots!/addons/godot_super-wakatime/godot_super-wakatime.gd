...
const HACKATIME_PROJECT_URL_FMT: String = \
	"https://hackatime.hackclub.com/api/v1/users/my/project/{project}"
...
var project_time_request: HTTPRequest
var current_time: String = "Hackatime"
...
func _disable_plugin() -> void:
	...
	if project_time_request:
		project_time_request.queue_free()
		project_time_request = null
...
func send_heartbeat(filepath: String, catagory: String, line_num: int, cursor_pos: int, lines: int, is_write: bool) -> void:
	...
	if is_write:
		cmd.append("--write")
	cmd.append_array(["--project", get_project_name()])
...
func _handle_heartbeat(cmd_arguments) -> void:
	...
	call_deferred("request_project_time")
...
func get_project_name() -> String:
	"""Return the canonical Hackatime project name for this repository."""
	return ProjectSettings.get_setting(
		"hackatime/project_name",
		ProjectSettings.get_setting("application/config/name")
	)


func request_project_time() -> void:
	"""Fetch this project's cumulative Hackatime time."""
	if project_time_request:
		return

	var api_key = get_api_key()
	if api_key == null:
		current_time = "Hackatime"
		_update_panel_label(current_time, "API key unavailable")
		return

	project_time_request = HTTPRequest.new()
	project_time_request.request_completed.connect(_on_project_time_received)
	add_child(project_time_request)

	var url := HACKATIME_PROJECT_URL_FMT.format({
		"project": get_project_name().uri_encode()
	})
	var headers := PackedStringArray([
		"Accept: application/json",
		"Authorization: Bearer %s" % api_key
	])
	var request_error := project_time_request.request(url, headers)
	if request_error != OK:
		Utils.plugin_print_err("Failed to request project time [Error: %s]" % request_error)
		project_time_request.queue_free()
		project_time_request = null


func _on_project_time_received(
	result: int,
	response_code: int,
	_headers: PackedStringArray,
	body: PackedByteArray
) -> void:
	var request := project_time_request
	project_time_request = null
	if request:
		request.queue_free()

	if result != HTTPRequest.RESULT_SUCCESS or response_code != 200:
		Utils.plugin_print_err(
			"Failed to fetch project time [Result: %s, HTTP: %s]" % [result, response_code]
		)
		return

	var response = JSON.parse_string(body.get_string_from_utf8())
	if not response is Dictionary or not response.has("total_seconds"):
		Utils.plugin_print_err("Hackatime returned an invalid project-time response")
		return

	current_time = format_duration(int(response["total_seconds"]))
	_update_panel_label(current_time, "%s: %s" % [get_project_name(), current_time])
	Utils.plugin_print("Project time updated: %s" % current_time)
